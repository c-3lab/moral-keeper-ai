...
class TestMoralKeeperAI:
    ...
    @pytest.mark.parametrize('model', ['gpt-4o', 'gpt-4o-mini', 'gpt-35-turbo'])
    ...
    def test_check_function_with_false_response(self, mock_azure_openai_client, model):
        ...
        with patch.dict(os.environ, {'AZURE_OPENAI_DEPLOY_NAME': model}):
            ...
            result = moral_keeper_ai.check(
                'お前みたいなバカがいるから、'
                'このプロジェクトはいつまでたっても進まないんだよ。いい加減にしろ！'
            )
    ...
    def test_suggest_function(self, mock_azure_openai_client):
        ...
        moral_keeper_ai = MoralKeeperAI()
        result = moral_keeper_ai.suggest(
            'このデータは古すぎて役に立たない。もっと新しいものをアップロードしてくれ。'
        )
        expected_result = (
            '公開されているデータが古いため、'
            '最新のデータをご提供いただけると大変助かります。例えば、'
            '（具体的な例：最新の人口統計情報や交通状況のデータ）などがあると、'
            'より有益です。どうぞご検討ください。'
        )
